...
import { adminListUsers, listCourts as listCourtsApi, listBookings as listBookingsApi, createCourt as createCourtApi, updateCourt as updateCourtApi, deleteCourt as deleteCourtApi, reorderCourts as reorderCourtsApi } from "@/lib/api";
...
function hasRevenueAttendanceRecord(booking: BookingLike) {
  return Boolean(
...
function getCourtAdminOrderValue(court: Court) {
  const value = Number((court as any).displayOrder)
  return Number.isFinite(value) && value > 0 ? value : Number.MAX_SAFE_INTEGER
}

function getCourtCreatedAtMs(court: Court) {
  const raw = (court as any).createdAt
  const value = raw instanceof Date ? raw.getTime() : new Date(raw || 0).getTime()
  return Number.isFinite(value) ? value : 0
}

function compareCourtsByAdminOrder(a: Court, b: Court) {
  return (
    getCourtAdminOrderValue(a) - getCourtAdminOrderValue(b) ||
    getCourtCreatedAtMs(b) - getCourtCreatedAtMs(a) ||
...
async function fetchRemainingPages(
  fetcher: (args: Record<string, any>) => Promise<{ items?: any[] }>,
  ...
  for (let page = 2; page <= totalPages; page += chunkSize) {
    const chunk = [];

    for (let offset = 0; offset < chunkSize && page + offset <= totalPages; offset++) {
      chunk.push(fetcher({ ...args, page: page + offset, limit }));
    }
...
function getCourtHours(court: Court) {
  const openTime = court.openTime || (court as any)?.schedule?.[0]?.openTime || "08:00";
  const closeTime = court.closeTime || (court as any)?.schedule?.[0]?.closeTime || "23:00";
...
function Pagination({
  ...
  return (
    <div className={cn("flex items-center justify-center gap-2 pt-4", rtl && "flex-row-reverse")}>
      <Button
        variant="outline"
...
export function AdminCourtsPage() {
  ...
  // ---------------------------------------------
  // Admin: manager assignment options (replace with API later)
  // ---------------------------------------------
  const [managerOptions, setManagerOptions] = useState<Array<{ id: string; nameEn: string; nameAr: string }>>([]);
  ...
  const [sortBy, setSortBy] = useState<SortBy>("display_order_asc");
  ...
  const loadData = useCallback(async () => {
    try {
      const courtLimit = 100;
      const managerLimit = 100;
      const [courtsRes, managersRes] = await Promise.all([
        listCourtsApi({ page: 1, limit: courtLimit }),
        adminListUsers({ role: "manager", page: 1, limit: 100 }),
      ]);

      let allCourts = [...(courtsRes.items || [])];
      let allManagers = [...(managersRes.items || [])];

      const cPages = (courtsRes as any).pagination?.pages || (courtsRes as any).pages || 1;
      const mPages = (managersRes as any).pagination?.pages || (managersRes as any).pages || 1;

      const [cResults, mResults] = await Promise.all([
        fetchRemainingPages(listCourtsApi, {}, cPages, courtLimit),
        fetchRemainingPages(adminListUsers, { role: "manager" }, mPages, managerLimit),
      ]);

      allCourts = allCourts.concat(cResults);
      allManagers = allManagers.concat(mResults);

      setCourtsState(allCourts as Court[]);
      setManagerOptions(allManagers.map((m: any) => ({
        id: m.id,
        nameEn: m.businessNameEn || m.name,
        nameAr: m.businessName || m.name,
      })));
    } catch (error) {
      console.error(error);
      setCourtsState([]);
  ...
  const loadPeriodBookings = useCallback(async () => {
    try {
      ...
      const bookingsRes = await listBookingsApi(bookingParams);
      let allBookings = [...(bookingsRes.items || [])];
      const bookingPages = bookingsRes.pages || 1;

      if (bookingPages > 1) {
        const extraBookings = await fetchRemainingPages(
          listBookingsApi,
          {
            dateFrom: bookingParams.dateFrom,
            dateTo: bookingParams.dateTo,
            sortBy: bookingParams.sortBy,
            order: bookingParams.order,
          },
          bookingPages,
          bookingLimit,
        );

        allBookings = allBookings.concat(extraBookings);
      }
  ...
  useAutoRefresh(async () => {
    await Promise.all([loadData(), loadPeriodBookings()]);
  });
  ...
  const adminOrderedVisibleCourts = useMemo(
    () => [...visibleCourts].sort(compareCourtsByAdminOrder),
    [visibleCourts],
  );

  const getCourtOrderPosition = useCallback(
    (courtId: string) => {
      const sourceIds = isReorderMode
        ? orderDraftIds
        : adminOrderedVisibleCourts.map((court) => court.id);
      const index = sourceIds.indexOf(courtId);
      return index >= 0 ? index + 1 : 0;
    },
    [adminOrderedVisibleCourts, isReorderMode, orderDraftIds],
  );

  const startReorderMode = useCallback(() => {
    const ids = adminOrderedVisibleCourts.map((court) => court.id);
    setOrderDraftIds(ids);
    setIsReorderMode(true);
    setSortBy("display_order_asc");
    setViewMode("grid");
    setSearchQuery("");
    setFilterStatus("all");
    setFilterCityEn("all");
    setPage(1);
  }, [adminOrderedVisibleCourts]);
  ...
  const moveCourtInOrder = useCallback((courtId: string, direction: "previous" | "next" | "first" | "last") => {
    setOrderDraftIds((prev) => {
      const currentIndex = prev.indexOf(courtId);
  ...
  const moveCourtBeforeTarget = useCallback((courtId: string, targetCourtId: string) => {
    if (courtId === targetCourtId) return;

    setOrderDraftIds((prev) => {
      const currentIndex = prev.indexOf(courtId);
      const targetIndex = prev.indexOf(targetCourtId);
      if (currentIndex < 0 || targetIndex < 0 || currentIndex === targetIndex) return prev;

      const next = [...prev];
      const [movedId] = next.splice(currentIndex, 1);
      const adjustedTargetIndex = next.indexOf(targetCourtId);
      next.splice(adjustedTargetIndex, 0, movedId);
      return next;
    });
  }, []);

  const handleCourtDragStart = useCallback((event: React.DragEvent<HTMLDivElement>, courtId: string) => {
    if (!isAdmin || !isReorderMode || isSavingOrder) return;
    setDraggingCourtId(courtId);
    setDragOverCourtId(null);
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", courtId);
  }, [isAdmin, isReorderMode, isSavingOrder]);

  const handleCourtDragOver = useCallback((event: React.DragEvent<HTMLDivElement>, courtId: string) => {
    if (!isAdmin || !isReorderMode || isSavingOrder || draggingCourtId === courtId) return;
    event.preventDefault();
    event.dataTransfer.dropEffect = "move";
    setDragOverCourtId(courtId);
  }, [draggingCourtId, isAdmin, isReorderMode, isSavingOrder]);

  const handleCourtDrop = useCallback((event: React.DragEvent<HTMLDivElement>, targetCourtId: string) => {
    if (!isAdmin || !isReorderMode || isSavingOrder) return;
    event.preventDefault();

    const sourceCourtId = draggingCourtId || event.dataTransfer.getData("text/plain");
    if (sourceCourtId) moveCourtBeforeTarget(sourceCourtId, targetCourtId);

    setDraggingCourtId(null);
    setDragOverCourtId(null);
  }, [draggingCourtId, isAdmin, isReorderMode, isSavingOrder, moveCourtBeforeTarget]);
  ...
  const saveCourtOrder = useCallback(async () => {
    if (!isAdmin) return;
    const expectedIds = adminOrderedVisibleCourts.map((court) => court.id);
    if (orderDraftIds.length !== expectedIds.length || new Set(orderDraftIds).size !== orderDraftIds.length) {
      toast.error(language === "ar" ? "ترتيب الملاعب غير مكتمل" : "Court order is incomplete");
      return;
    }

    setIsSavingOrder(true);
    try {
      const res = await reorderCourtsApi(orderDraftIds);
      setCourtsState(Array.isArray(res.items) ? (res.items as Court[]) : []);
      setIsReorderMode(false);
      setOrderDraftIds([]);
      setDraggingCourtId(null);
      setDragOverCourtId(null);
      toast.success(language === "ar" ? "تم حفظ ترتيب الملاعب" : "Court order saved");
    } catch (error: any) {
      toast.error(error?.message || (language === "ar" ? "تعذر حفظ الترتيب" : "Could not save court order"));
    } finally {
      setIsSavingOrder(false);
    }
  }, [adminOrderedVisibleCourts, isAdmin, language, orderDraftIds]);
  ...
  const courtStatsMap = useMemo(() => {
    ...
    visibleCourts.forEach((court) => {
      ...
      const revenue = courtBookings.reduce((sum, b) => {
        const amount = Number(b.totalPrice ?? b.amount ?? 0);
        return hasRevenueAttendanceRecord(b) ? sum + amount : sum;
      }, 0);
      ...
      const nextBooking = visibleBookings
        ...
        .map((b) => ({
          b,
          dt: new Date(`${b.date}T${b.startTime || "00:00"}:00`),
        }))
        .filter((x) => x.dt.getTime() >= now.getTime())
        .sort((a, b) => a.dt.getTime() - b.dt.getTime())[0]?.b;
  ...
  const totals = useMemo(() => {
    ...
    const revenue = visibleCourts.reduce(
      (sum, c) => sum + (courtStatsMap[c.id]?.revenue || 0),
      0,
    );
    ...
    return { courts, active, maintenance, revenue, bookings, occupancyAvg };
  ...
  const filteredCourts = useMemo(() => {
    let courts = isReorderMode && isAdmin
      ? orderDraftIds
          .map((id) => visibleCourts.find((court) => court.id === id))
          .filter(Boolean) as Court[]
      : [...visibleCourts];

    if (isReorderMode && isAdmin) return courts;
    ...
    withStats.sort((a, b) => {
      ...
      if (sortBy === "display_order_asc") return compareCourtsByAdminOrder(a.c, b.c);
      return 0;
    });
  ...
  const totalPages = isMobile || (isAdmin && isReorderMode)
    ? 1
    : Math.max(1, Math.ceil(filteredCourts.length / pageSize));
  const pagedCourts = useMemo(() => {
    if (isMobile || (isAdmin && isReorderMode)) return filteredCourts;
    const start = (page - 1) * pageSize;
    return filteredCourts.slice(start, start + pageSize);
  }, [filteredCourts, isAdmin, isMobile, isReorderMode, page, pageSize]);
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 6) {
      if (!formData.managerId) {
        toast.error(
          language === "ar" ? "اختر مديرًا للملعب" : "Assign a manager",
        );
        return false;
      }
    }
  ...
  const isStepComplete = (step: number) => {
    ...
    if (step === 6) return !!formData.managerId;
    return true;
  };

  const toCourtFromForm = (base?: Court): Partial<Court> => {
    ...
    // Reorder images so cover is at index 0
    let finalImages = [...images];
    if (formData.coverImageIndex > 0 && formData.coverImageIndex < finalImages.length) {
      const cvr = finalImages.splice(formData.coverImageIndex, 1)[0];
      finalImages.unshift(cvr);
    }
  ...
  const createCourt = async () => {
    ...
    try {
      const res = await createCourtApi(court as any);
      setCourtsState((prev) => [...prev, res.court as Court]);
  ...
  const updateCourt = async (id: string) => {
    ...
    try {
      ...
      setCourtsState((prev) => prev.map((c) => (c.id === id ? (res.court as Court) : c)));
  ...
  const handleCreateNextOrSave = async () => {
    ...
    const success = await createCourt();
    if (success) {
  ...
  const handleEditNextOrSave = async () => {
    ...
    const success = await updateCourt(selectedCourt.id);
    if (success) {
  ...
  const handleDeleteCourt = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteCourtApi(selectedCourt.id);

      toast.success(language === "ar" ? "تم حذف الملعب نهائياً" : "Court permanently deleted");
  ...
  const renderStepNav = () => {
    const items = [
      ...
      language === "ar" ? "تعيين المدير" : "Manager",
    ];
    return (
      <div
      ...
      <Card className="rounded-2xl border-border/60 bg-background/60 backdrop-blur">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline" className="rounded-2xl">
              {(getManagerName(formData.managerId) || (isAr ? "غير محدد" : "Unassigned"))}
            </Badge>
          </div>
    ...
    switch (currentStep) {
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="display_order">
                  {isAr ? "ترتيب العرض (الأعلى يظهر أولاً)" : "Display Order (Highest appears first)"}
                </Label>
                <Input
                  id="display_order"
                  type="number"
      ...
      case 2:
        return (
          <div className="space-y-6">
            ...
            <Alert className="border-border/60 bg-muted/20 [&>div[data-slot=alert-description]]:col-start-auto [&>div[data-slot=alert-description]]:gap-0.5 [&>span]:translate-y-0">
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-[4/3] rounded-xl overflow-hidden border border-border/60 bg-muted group/img">
                        ...
                        {/* Top-Right Quick Actions (Glassmorphism) */}
                        ...
                        {/* Bottom Overlay Actions (Glassmorphism) */}
                      ...
                      {/* Fallback actions for touch devices/visible by default on mobile or specific state */}
      ...
      case 6:
        return (
          <div className="space-y-6">
            <div className="grid gap-2">
              <Label>{isAr ? "تعيين مدير *" : "Assign manager *"}</Label>
              <Select
              ...
              {!!formData.managerId && (
                <p className="text-xs text-muted-foreground">
                  ...
                  <span className="font-medium">
                    {getManagerName(formData.managerId)}
                  </span>
                </p>
              )}
            </div>
            ...
            <Alert className="border-border/60 bg-muted/20">
              ...
              <AlertDescription className="text-sm">
                {isAr
                  ? "هذا الحقل خاص بالـ Admin لتحديد مدير المسؤول عن الملعب."
  ...
  // ---------------------------------------------
  const renderCourtCard = ({ court, imagePriority = false }: { court: Court; imagePriority?: boolean }) => {
    ...
    const orderPosition = getCourtOrderPosition(court.id);

    return (
      <div
        draggable={isAdmin && isReorderMode && !isSavingOrder}
        onDragStart={(e) => handleCourtDragStart(e, court.id)}
        onDragOver={(e) => handleCourtDragOver(e, court.id)}
        onDragEnter={(e) => handleCourtDragOver(e, court.id)}
        onDrop={(e) => handleCourtDrop(e, court.id)}
        onDragEnd={handleCourtDragEnd}
      >
      <MotionCard
        layout={!isMobile && !isReorderMode}
        whileHover={isMobile || prefersReducedMotion ? undefined : { y: -4 }}
        transition={{ type: "spring", stiffness: 260, damping: 22 }}
        className={cn(
          "group overflow-hidden border-border/50 hover:shadow-lg hover:shadow-primary/5 transition-shadow",
          isReorderMode ? "cursor-grab active:cursor-grabbing ring-1 ring-primary/20" : "cursor-pointer",
          isReorderMode && draggingCourtId === court.id && "opacity-60 scale-[0.99]",
          isReorderMode && dragOverCourtId === court.id && draggingCourtId !== court.id && "ring-2 ring-primary ring-offset-2 ring-offset-background",
        )}
        onClick={(e) => {
          if (e.defaultPrevented || isReorderMode) return;
          openView(court);
        }}
      >
        <div className="relative aspect-[16/9] bg-muted overflow-hidden">
          ...
          {isAdmin && isReorderMode && (
            <div
              className="absolute inset-x-3 top-14 z-30 rounded-2xl border border-primary/25 bg-background/95 p-2 shadow-lg backdrop-blur-md"
              onPointerDown={(e) => e.stopPropagation()}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-xl bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">
                  <GripVertical className="h-3.5 w-3.5" />
                  {language === "ar" ? "الترتيب" : "Order"} #{orderPosition || "-"}
                </span>
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    disabled={orderPosition <= 1 || isSavingOrder}
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      moveCourtInOrder(court.id, "first");
                    }}
                    aria-label={language === "ar" ? "نقل إلى البداية" : "Move to first"}
                  >
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      moveCourtInOrder(court.id, "previous");
                    }}
                    aria-label={language === "ar" ? "تحريك للخلف" : "Move left / earlier"}
                  >
                  ...
                  <Button
                    ...
                    size="icon"
                    className="h-8 w-8 rounded-xl bg-background/70"
                    disabled={orderPosition <= 0 || orderPosition >= orderDraftIds.length || isSavingOrder}
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      moveCourtInOrder(court.id, "next");
                    }}
                    aria-label={language === "ar" ? "تحريك للأمام" : "Move right / later"}
                  >
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      moveCourtInOrder(court.id, "last");
                    }}
                    aria-label={language === "ar" ? "نقل إلى النهاية" : "Move to last"}
                  >
              ...
              <p className="mt-1 px-1 text-[11px] leading-4 text-muted-foreground">
                {language === "ar"
                  ? "اسحب البطاقة أو استخدم الأسهم: اليسار = أبكر، اليمين = لاحق."
                  : "Drag the card or use arrows: left = earlier, right = later."}
              </p>
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {stats?.nextBooking && (
              <Badge variant="outline" className="rounded-2xl bg-muted/30">
                ...
                {stats.nextBooking.date} {stats.nextBooking.startTime}
              </Badge>
            )}
          </div>
  ...
  // ---------------------------------------------
  const renderCourtViewDialog = () => {
    // 1. Declare ALL hooks at the absolute top of the component!
    // No hooks should be declared inside conditional statements.
    // However, since we're calling this inside the render of AdminCourtsPage, we should lift state up.

    const c = selectedCourt as any;

    // 2. SAFE early return AFTER the hooks
    if (!c) return null;
            ...
            <Tabs dir={language === "ar" ? "rtl" : "ltr"} defaultValue="overview" className="space-y-5">
                ...
                <div className="grid lg:grid-cols-[1fr,360px] gap-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <InfoRow
                      ...
                      label={isAr ? "المدير" : "Manager"}
                      value={
                        (c as any).managerName ||
                        getManagerName((c as any).managerId) ||
                        (isAr ? "غير محدد" : "Unassigned")
                      }
                      secondary={
                        (c as any).managerId
                          ? `${isAr ? "ID" : "ID"}: ${(c as any).managerId}`
                          : ""
                      }
                    />
              ...
              <TabsContent value="performance" className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-4">
                  ...
                  <KpiCard title={isAr ? "الإيراد" : "Revenue"} value={fmtMoney(stats?.revenue || 0)} hint={t("common.egp")} />
                  ...
                  <Alert className="border-border/60 bg-muted/20">
                    ...
                    <AlertDescription className="text-sm">
                      ...
                      <span className="font-semibold">
                        {stats.nextBooking.date} • {stats.nextBooking.startTime} - {stats.nextBooking.endTime}
                      </span>
                    </AlertDescription>
                  </Alert>
                )}
    ...
    <div className="space-y-6" dir={language === "ar" ? "rtl" : "ltr"}>
      <Card className="relative overflow-hidden rounded-[1.75rem] border-border/60 bg-card shadow-sm">
        ...
        <CardContent className="p-5 sm:p-6 relative">
          <PageHeader
            ...
            breadcrumbs={[
              { label: t("dashboard.title"), href: isAdmin ? "/dashboard/admin" : "/dashboard/manager" },
              {
                label: isAdmin
                  ? language === "ar"
                    ? "الملاعب"
                ...
                <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
                  {isAdmin && !isReorderMode && (
                    <Button
                      ...
                      disabled={!adminOrderedVisibleCourts.length}
                      title={language === "ar" ? "ترتيب الملاعب" : "Reorder courts"}
                    >
                      <GripVertical className="h-4 w-4 me-2" />
                      <span className="truncate">
                        {language === "ar" ? "ترتيب الملاعب" : "Reorder courts"}
                      </span>
                    </Button>
                  )}
                  ...
                  {isAdmin && isReorderMode && (
                    <>
                      <Button
                        className="h-11 min-w-0 rounded-2xl px-3 text-sm sm:h-10 sm:flex-none sm:px-4"
                        onClick={saveCourtOrder}
                        disabled={isSavingOrder || !orderDraftIds.length}
                        title={language === "ar" ? "حفظ الترتيب" : "Save order"}
                      >
                        {isSavingOrder ? (
                  ...
                  <Button
                    ...
                    disabled={!filteredCourts.length || isReorderMode}
          ...
          <AlertDescription>
            <div className="flex flex-col gap-1">
              <span className="font-semibold text-foreground">
                {language === "ar" ? "وضع ترتيب الملاعب" : "Court reorder mode"}
              </span>
              <span>
                {language === "ar"
                  ? "اسحب أي بطاقة إلى مكانها الجديد، أو استخدم أزرار اليسار/اليمين على البطاقة. اليسار يجعل الملعب يظهر أبكر، واليمين يجعله يظهر لاحقاً."
                  : "Drag any card to its new position, or use the left/right buttons on the card. Left moves a court earlier; right moves it later."}
              </span>
              <span className="text-xs text-muted-foreground">
                {language === "ar"
                  ? "يتم عرض كل الملاعب في صفحة واحدة أثناء الترتيب، ويتم تعطيل البحث والفلاتر لتجنب حفظ ترتيب ناقص."
                  : "All courts are shown on one page while reordering, and search/filters are disabled to avoid saving an incomplete order."}
              </span>
            </div>
          </AlertDescription>
        </Alert>
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-5">
        ...
        <AnimatedContainer animation="fade-up" delay={160}>
          <KpiCard
            ...
            hint={t("common.egp")}
            className="bg-gradient-to-br from-info/10 via-info/5 to-transparent"
          />
        </AnimatedContainer>
      </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex gap-2">
                <Select value={filterStatus} onValueChange={setFilterStatus} disabled={isReorderMode}>
                ...
                <Select value={sortBy} onValueChange={(v) => setSortBy(v as SortBy)} disabled={isReorderMode}>
                  <SelectTrigger className="h-11 rounded-2xl flex-1 sm:w-[160px] bg-background">
                    <SelectValue placeholder={language === "ar" ? "ترتيب" : "Sort"} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="revenue_desc">{language === "ar" ? "الإيراد الأعلى" : "Revenue (high)"}</SelectItem>
                    ...
                    <SelectItem value="display_order_asc">{language === "ar" ? "ترتيب الإدارة" : "Admin order"}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
      ...
      <Dialog open={advancedFiltersOpen} onOpenChange={setAdvancedFiltersOpen}>
        <DialogContent className="rounded-3xl sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>
              {language === "ar" ? "فلاتر إضافية" : "Advanced filters"}
            </DialogTitle>
            <DialogDescription>
              {language === "ar"
                ? "تحكم أدق في عرض الملاعب"
          ...
          <div className="space-y-4">
            <div className="grid gap-2">
              ...
              <Select value={filterCityEn} onValueChange={setFilterCityEn}>
                <SelectTrigger className="rounded-2xl">
                  <SelectValue
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label>
                  {language === "ar" ? "موثق فقط" : "Only verified"}
                </Label>
                <p className="text-xs text-muted-foreground">
                  {language === "ar"
                    ? "إظهار الملاعب الموثقة"
                    : "Show only verified courts"}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label>
                  {language === "ar" ? "مميز فقط" : "Only featured"}
                </Label>
                <p className="text-xs text-muted-foreground">
                  {language === "ar"
                    ? "إظهار الملاعب المميزة"
                    : "Show only featured courts"}
                </p>
              </div>
              <span className="sr-only" />
            </div>
      ...
      {/* Courts Area */}
      <div className="p-4 sm:p-5 bg-muted/5 flex-1 flex flex-col">
        {hasNoCourts ? (
          <EmptyState
            icon={Building2}
              ...
              >
                {isMobile ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-muted-foreground">
                        {isReorderMode
                          ? language === "ar"
                            ? "رتّب الملاعب بالأسهم أو بالسحب"
                            : "Reorder with arrows or drag"
                          : language === "ar"
                            ? "اسحب للتنقل بين الملاعب"
                            : "Swipe through courts"}
                      </p>
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
                          className="rounded-2xl bg-transparent"
                          onClick={() =>
                            scrollSlider(language === "ar" ? "next" : "prev")
                          }
                        >
                          {language === "ar" ? (
                        ...
                        <Button
                          ...
                          className="rounded-2xl bg-transparent"
                          onClick={() =>
                            scrollSlider(language === "ar" ? "prev" : "next")
                          }
                        >
                          {language === "ar" ? (
                            <ChevronLeft className="h-4 w-4" />
                    ...
                    <div
                      ref={sliderRef}
                      className="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory scroll-smooth [-webkit-overflow-scrolling:touch]"
                    >
                      {pagedCourts.map((court, index) => (
                        <div
                          key={`slider-${court.id}`}
                          className="snap-center shrink-0 w-[88vw] max-w-[520px]"
                        >
                          {renderCourtCard({ court, imagePriority: index === 0 })}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    {pagedCourts.map((court, index) => (
                      <div key={`grid-${court.id}`}>{renderCourtCard({ court, imagePriority: index === 0 })}</div>
                    ))}
                  </div>
                )}
              </motion.div>
                ...
                {pagedCourts.map((court) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      className={cn(
                        "w-full text-left group flex items-center justify-between p-4 rounded-3xl",
                        "border border-border/50 bg-background/70 hover:bg-muted/30 transition-colors",
                      )}
                      onClick={() => openView(court)}
                    >
                      <div className="flex items-center gap-4 min-w-0">
                        <div className="relative h-14 w-24 rounded-2xl overflow-hidden border border-border/50 bg-muted shrink-0">
                          <NextImage
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground truncate mt-1">
                            ...
                            •{" "}
                            {language === "ar"
                              ? (sportTypes as any)[(court as any).sportType]
                                ?.ar
                              : (sportTypes as any)[(court as any).sportType]
                                ?.en}
                          </p>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Badge
                          variant="outline"
                          className="rounded-2xl bg-muted/30 border-border/50 hidden sm:inline-flex"
                        >
                          {language === "ar" ? "حجوزات" : "Bookings"}:{" "}
                          {s?.bookings || 0}
                        </Badge>
                        <Badge
                          variant="outline"
                          className="rounded-2xl bg-muted/30 border-border/50 hidden md:inline-flex"
                        >
                          {language === "ar" ? "إيراد" : "Revenue"}:{" "}
